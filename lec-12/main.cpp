...
void addboundary (std::vector<std::pair<int, int>> &origin)
{
    std::vector<std::pair<int, int>> temp;
    if (origin.size() != 0)
    {
        std::pair<int, int> firstelement;
        std::pair<int, int> lastelement;
        int vh;
        int originlength = origin.size();
        int x1 = origin.at(0).first;
        int x2 = origin.at(1).first;
        int y = origin.at(0).second;
        if (x1 == x2)
        {
            vh = 1;
            if (y-1 >= 0)
            {
                firstelement = std::make_pair(x1, y-1);
                origin.insert(origin.begin(), firstelement);
            }
            if (y+originlength < rowSize)
            {
                lastelement = std::make_pair(x1, y+originlength);
                origin.insert(origin.end(), lastelement);
            }
        }
        else
        {
            ...
            if (x1 -1 >= 0)
            {
                firstelement = std::make_pair(x1-1, y);
                origin.insert(origin.begin(), firstelement);
            }
            if (x1+originlength < columnSize)
            {
                lastelement = std::make_pair(x1+originlength, y);
                origin.insert(origin.end(), lastelement);
            }
        }
        ...
        while(i != origin.end())
        {
            if (vh == 0)
            {
                ...
                if (y-1 >= 0)
                {
                    temp.push_back(std::make_pair(x, y-1));
                }
            ...
            else
            {
                ...
                if (x1-1 >= 0)
                {
                    temp.push_back(std::make_pair(x1-1, y2));
                }
...
bool isOverlap (std::vector<std::pair<int, int>> &epoints)
{
    ...
    while (j != board.end())
    {
        ...
        while (i != column.end())
        {
            std::vector<std::pair<int, int>> origin = (*i)->getinfo();
            addboundary(origin);
            for (auto &n:origin)
            {
                for (auto &m:epoints)
